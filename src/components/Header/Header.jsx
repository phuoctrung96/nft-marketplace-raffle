  ...
  return (
    <div className='z-50 flex items-center justify-between w-full px-5 py-4 font-normal h-18 md:px-12'>
      ...
      {isNavOpen ? (
        ...
        <Hamburger className='cursor-pointer md:hidden' onClick={() => setIsNavOpen(!isNavOpen)} />
      )}
      <div className='items-center hidden  md:flex desktop-nav'>
        ...
        {account ? (
          <Button text={parseAddress(account)} accent additionalClass='px-10 ml-20' />
        ) : (
          <Button text='Connect' onClickHandler={connectWallet} additionalClass='ml-20' accent />
        )}
      </div>
      {isNavOpen && (
        <div
